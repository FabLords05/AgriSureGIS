...
class GetParserSettingsTests(unittest.TestCase):
    def test_creates_default_row_if_missing(self):
        ...
        def refresh_side_effect(obj):
            obj.polling_interval_minutes = 180
        mock_db.refresh.side_effect = refresh_side_effect

        result = get_parser_settings(db=mock_db)

        self.assertEqual(result, {"polling_interval_minutes": 180})
    ...
    def test_returns_existing_value(self):
        mock_db = MagicMock()
        mock_db.query.return_value.first.return_value = ParserSettings(polling_interval_minutes=360)

        result = get_parser_settings(db=mock_db)

        self.assertEqual(result, {"polling_interval_minutes": 360})
...
class UpdateParserSettingsTests(unittest.TestCase):
    ...
    def test_persists_new_value_and_reschedules(self, mock_reschedule):
        existing = ParserSettings(polling_interval_minutes=180)
        mock_db = MagicMock()
        mock_db.query.return_value.first.return_value = existing
        mock_scheduler = MagicMock()

        result = update_parser_settings(
            payload=ParserSettingsUpdate(polling_interval_minutes=15),
            request=_fake_request(scheduler=mock_scheduler),
            db=mock_db,
        )

        self.assertEqual(existing.polling_interval_minutes, 15)
        mock_db.commit.assert_called_once()
        mock_reschedule.assert_called_once_with(mock_scheduler, 15)
        self.assertEqual(result, {"polling_interval_minutes": 15})

    def test_rejects_value_below_15(self):
        with self.assertRaises(pydantic.ValidationError):
            ParserSettingsUpdate(polling_interval_minutes=14)

    def test_rejects_value_above_1440(self):
        with self.assertRaises(pydantic.ValidationError):
            ParserSettingsUpdate(polling_interval_minutes=1441)
