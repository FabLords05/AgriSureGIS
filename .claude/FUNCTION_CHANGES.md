...
### Status / Next Steps
* **Conflict risk flagged, not yet resolved:** this branch's tip (`425e129`, Fabio's) already modifies `frontend/src/lib/api.ts`; this entry's commit also modifies it (`InsuranceSummary` type, `Farm` fields). Likely auto-mergeable (both additive), but needs verification at merge time.
